...
/// Placeholder for the reserved `ASIDControl` arch kind (the renamed `ASID`,
/// 2026-09-21): the capability-protected control point over the ASID
/// namespace — the expected home for pool creation/partitioning (seL4-style).
///
/// An ASID is a hardware naming value bound to an `AddressSpace`, not an
/// independently capabilitied object, so the former `ASID` kind was replaced
/// by `ASIDControl`. Its operation schema is open; the kind stays
