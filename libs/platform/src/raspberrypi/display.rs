...
impl Display {
    ...
    #[inline]
    fn color_component(&self, chan: Chan) -> u32 {
    ...
    #[inline]
    fn write_pixel_component(&self, x: u32, y: u32, chan: Chan, c: u8) {
        // SAFETY: Trust me, it's safe!
        unsafe {
            *(self.base as *mut u8).offset(
                (y * self.pitch + x * (self.depth >> 3) + self.color_component(chan))
                    .cast_signed()
                    .try_into()
                    .unwrap(),
            ) = c;
    ...
    pub fn putpixel(&mut self, x: u32, y: u32, color: u32) {
        self.write_pixel_component(x, y, Chan::R, (color & 0xff).try_into().unwrap());
        self.write_pixel_component(x, y, Chan::G, ((color >> 8) & 0xff).try_into().unwrap());
        self.write_pixel_component(x, y, Chan::B, ((color >> 16) & 0xff).try_into().unwrap());
    }
    ...
    pub fn draw_text(&mut self, x: u32, y: u32, text: &str, color: Color) -> Result<()> {
        for i in 0..8 {
            ...
            for (char_off, my_char) in text.as_bytes().iter().enumerate() {
                let off = u32::try_from(char_off * 8).unwrap();
                let my_char = *my_char;

                if (my_char > /*NUM_CHARS*/95 + 0x20) || (my_char.cast_signed() - 0x20 < 0) {
                    return Err(DrawError::CharacterNotInFont);
                }

                // SAFETY: We have just checked the bounds above.
                let mut myval = unsafe { *CHAR_ARRAY.get_unchecked(my_char as usize - 0x20) };
