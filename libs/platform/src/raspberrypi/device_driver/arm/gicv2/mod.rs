...
impl GICv2 {
    ...
    pub const unsafe fn new(
        gic_distr_mmio_start_addr: Address<Virtual>,
        gic_ctrlr_mmio_start_addr: Address<Virtual>,
    ) -> Self {
        Self {
            // SAFETY: SAFE
            gicd: unsafe { gicd::GICD::new(gic_distr_mmio_start_addr) },
            // SAFETY: SAFE
            gicc: unsafe { gicc::GICC::new(gic_ctrlr_mmio_start_addr) },
...
impl libdriver::drivers::interface::DeviceDriver for GICv2 {
    ...
    unsafe fn init(&self) -> Result<(), &'static str> {
        if BOOT_CORE_ID == libcpu::smp::core_id() {
...
impl IRQManager for GICv2 {
    ...
    fn handle_pending_irqs<'irq_context>(&'irq_context self, ic: &IRQContext<'irq_context>) {
        ...
        self.gicc
            .mark_completed(u32::try_from(irq_number).unwrap(), ic);
    }
