...
use {
    aarch64_cpu::registers::{ESR_EL1, FAR_EL1, SPSR_EL1},
    liblog::println,
...
pub fn exception_dump(e: &mut super::ExceptionContext) -> bool {
    ...
    if cause == ESR_EL1::EC::DataAbortCurrentEL.read(ESR_EL1::EC) {
        ...
        #[rustfmt::skip]
        {
            ...
            println!("     SPSR_EL1: {:#016x} (state)", e.spsr_el1.0.get());
            println!("               N: {} (negative condition)", e.spsr_el1.0.read(SPSR_EL1::N));
            println!("               Z: {} (zero condition)", e.spsr_el1.0.read(SPSR_EL1::Z));
            println!("               C: {} (carry condition)", e.spsr_el1.0.read(SPSR_EL1::C));
            println!("               V: {} (overflow condition)", e.spsr_el1.0.read(SPSR_EL1::V));
            println!("               SS: {} (software step)", e.spsr_el1.0.read(SPSR_EL1::SS));
            println!("               IL: {} (illegal execution state)", e.spsr_el1.0.read(SPSR_EL1::IL));
            println!("               D: {} (debug masked)", e.spsr_el1.0.read(SPSR_EL1::D));
            println!("               A: {} (serror masked)", e.spsr_el1.0.read(SPSR_EL1::A));
            println!("               I: {} (irq masked)", e.spsr_el1.0.read(SPSR_EL1::I));
            println!("               F: {} (fiq masked)", e.spsr_el1.0.read(SPSR_EL1::F));
            println!("               M: {:#06b} (machine state)", e.spsr_el1.0.read(SPSR_EL1::M));
        }
    }
    ...
    {
        println!(
            "      Incrementing ELR_EL1 by 4 to continue with the first instruction after the exception!"
        );

        e.elr_el1 += 4;
        e.elr_el1 &= !0x1; // Clear low bit

        println!("      ELR_EL1 modified: {:#010x} (return to)", e.elr_el1);
        println!("      Returning from exception...\n");
        return true;
        // @todo: qemu::exit_success() from here directly?
    }
