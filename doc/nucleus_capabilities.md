...
Implementation status: `KeyTableOp` provides checked `TryFrom<u64>` and `TryFrom<u32>` decoding of the existing IDs above. The full-width decoder rejects every other value with the existing `InvalidOperation` error, including unassigned `3` and high-bit aliases; the `u32` adapter widens before delegating. The CopyDerive/Move/Delete handler is now active for the caller's own table (named through `CAPTBL_SELF`) with the approved rights matrix and wire schemas; Revoke returns `InvalidOperation`, and KeyTable capabilities naming other table objects are rejected as unsupported pending pooled-KeyTable resolution. Exception-origin validation remains unfinished.
