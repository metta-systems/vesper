...
These are contract decisions, not completed implementation tasks. New key/result/bootstrap encodings require a coordinated kernel/userspace migration; the selected logical field widths do not themselves implement wire packing or change error numbers or existing operation/type IDs.
...
- **Slot exhaustion (3B=A):** once a slot cannot issue a fresh 32-bit incarnation without reuse/wrap, prohibit further installation into that slot. Existing-capability deletion remains possible, and other slots remain usable. Do not retire the entire table merely because one slot exhausts. Incarnation advancement/initial values and exhaustion error encoding still need specification; table replacement must not reset stale-key protection.
- **Inconsistency (3C=B):** use one shared error with diagnostic reasons distinguishing stale slot incarnation, capability invalidation and underlying object retirement. Do not return a replacement key or silently refresh/retry. Numeric status/reason values and precedence when multiple conditions apply remain D9; ordinary bounds/rights failures retain their own semantics.
...
- **Separate table permissions (4B=B):** distinguish source derivation, source removal/move, destination installation and deletion authority rather than one all-or-nothing Manage permission. Final names, bit assignments and exact combinations remain to be specified; per-kind restrictions and rights attenuation still apply.

The following logical schemas retain operation IDs and require atomic, ownership-preserving failure behavior; they do not select register packing:
...
| D9 | DebugConsole gate preserves type `127` and Write `0`; no new inline-byte ABI is approved. Inconsistency with diagnostic reasons is selected for stale/invalidated capabilities and retired objects, without automatic refresh; its status/reason numbers and precedence remain open. Minimal KeyTable logical schemas are selected, not register packing. Aborted-work terms are adopted vocabulary only. Wire encodings, other shared errors/details, reserved/unsupported operations, ABI version/support discovery, and migration policy remain open. | Freezing new operation schemas or separately deployed ABI consumers |
