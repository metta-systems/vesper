...
Implementation status: `KeyTableOp` provides checked `TryFrom<u64>` and `TryFrom<u32>` decoding of the existing IDs above. The full-width decoder rejects every other value with the existing `InvalidOperation` error, including unassigned `3` and high-bit aliases; the `u32` adapter widens before delegating. This is shared ABI vocabulary only: the excluded KeyTable handler is not enabled, active dispatch still reports `UnsupportedCoreType(KeyTable)` after successful lookup, while exception-origin validation remains unfinished. The approved key package now supplies the key/CopyDerive schemas; it does not enable handlers or finalize their rights matrix.
...
The following logical schemas retain operation IDs and require atomic, ownership-preserving failure behavior; CopyDerive register packing is now selected by the approved key package; Move/Delete's remaining wire schemas are still pending:
...
The active debug fixture obtains its actual installed console key through `nucleus_bootstrap_debug_console`, a private debug-gated, one-shot `unsafe extern "C" fn() -> u64` bridge from trusted EL1 Kickstart. Its linked executable address is extracted from the paired nucleus ELF and retained in loaded `.text`; feature-off Kickstart neither extracts nor calls it. No SVC bootstrap operation, slot-to-key refresh, or guessed incarnation is introduced. The fixture's private Domain backing is a statically aligned `MaybeUninit<Domain>` in the reserved nucleus BSS, sized using `size_of::<Domain>()`, replacing the untracked `0x1000`/16-KiB pool. The current loader accounts the full BSS section and its mappings. This is boot-fixture backing, not implementation of general Untyped-backed pools or hostile-EL0 isolation. `just test-capability-boot` runs QEMU directly: in-guest assertions validate actual key handoff, successful debug Write through SVC, and exact zero-incarnation rejection. Semihosting exits with success after boot completes or failure on panic; no host log parser or recipe-level timeout is used.
...
| D3 | Selected: caller-local slot/incarnation keys with implicit table context; initial 32-bit slot plus 32-bit incarnation (optional type would take 8 slot bits); constant-controlled fixed capacity, runtime resizing out of initial scope; independent object identity; slot-local exhaustion without wrap and with cleanup permitted; derive rather than mutate authority in place initially; Move yields a destination-local key; inconsistency with diagnostic reasons; enforced single-core guarded access, SMP deferred; Untyped-backed kernel-private KeyTables; no automatic leak recovery; correctness-first layouts; intended private MappedSlice cap. Selected follow-up: packed incarnation-high/slot-low 64-bit key, no initial type tag, increment-on-install from 1, retained counters on deletion, no live-Domain logical table rebinding, non-owning typed handles, statuses 26–28 with diagnostic reasons, and 64-bit kernel allocation generations without wrap. Open: domain lifecycle integration, future in-place authority-mutation rules, metadata/backing ownership and concrete guarded access, unsafe full-borrow/fbuf protocols, per-kind details, and interim Frame slot no-reuse scope. Stronger stale-raw-pointer protection across VA reuse is deferred. | General capability access, domain teardown, safe client ownership APIs |
