...
| `just test-capability-boot` | Passed actual boot: Retype through the real SVC path, cross-table CopyDerive into the new table, the three failure cases (invalid kind, occupied slot, insufficient memory), and a second Retype plus post-carve derivation verifying consecutive-carve non-overlap |
...
Coverage limits: device-memory restrictions are not yet validated (no device Untyped exists; `is_device` is unchecked in Retype). Sanitization before cross-boundary exposure, frames, mapping, ASID, revocation/reclamation, and retirement transitions remain later Phase 5 work; only the `KeyTable` kind is creatable. The defensive mid-run insert-failure rollback is pre-validated away rather than separately exercised. The exact predefined-Domain list, capacities, and incarnation-bearing handoff records remain Phase 4 items. Next prerequisite is the frame/page-table backing and mapping vertical slice.
