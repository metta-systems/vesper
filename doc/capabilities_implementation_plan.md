...
- [ ] Decide Buffer's kernel-versus-userspace role while retaining its registered ID; settle mapping-context identity and ASID capability versus VSpace-binding ownership (D6). (2026-09-15: mapping-context identity selected — the Domain is the mapping context, no separately targetable VSpace object, registered VSpace ID stays reserved; ASID model selected — explicit capability-protected ASIDPool/ASID resources with authorized AssignASID-style binding to the Domain's translation root. Buffer's kernel-versus-userspace role remains open pending a maintainer follow-up.)
