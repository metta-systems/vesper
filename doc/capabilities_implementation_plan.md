...
- [x] Define open/closed wait identity, separate send and receive/reply timeouts, clock/units, no-wait/infinite encodings, cancellation, and late completion. (2026-09-16: selected and recorded in the contract — closed-wait identity is the kernel pending-invocation record with a non-wrapping 64-bit generation; one relative `u64`-nanosecond timeout per blocking operation with `u64::MAX` = infinite and zero invalid (no-wait via the nonblocking/Poll variants only, no absolute deadlines initially); a single Call timeout with phase-specific outcomes (send-phase = cancelled-before-commit, reply-phase = outcome-unknown); late replies consume the one-shot Reply with a defined replier error; cancellation by timeout and domain teardown only, scheduler-driven cancellation deferred to Phase 7; notification delivery is one-consumer; the terminal-transition rule (reply/timeout/cancellation/teardown compete for exactly one terminal transition) is adopted into the contract from lifetime-and-authority §7. Shared wire encodings for the new cancellation/late-reply errors remain D9 work; implementation remains open.)
- [ ] Apply the adopted rejected-before-admission / cancelled-before-commit / completed / outcome-unknown vocabulary to each local operation's commit guarantees; define wire representations separately. Keep remote/distributed protocols and policy out of the nucleus. (2026-09-18: `Notification.Wait`'s commit point is explicit — the pending record's enforced single terminal transition, delivered at resume; the remaining operations and the D9 wire encodings stay open.)
- [x] Implement explicit completed/blocked/handoff outcomes with saved pending invocation state; schedule only after relevant borrows/guards have ended. (2026-09-18: `InvokeOutcome::Complete`/`Blocked` at dispatch; the SVC entry parks a blocked caller's exception frame on its kernel stack, records `ExecutionContext::Parked` with the pending record, and switches through the bounded runnable-domain scheduler only after the kernel-lock closure has ended; resume delivers the terminal result into the parked frame and releases the record. A direct-handoff fast path remains an unimplemented optimization of the same boundary. Validation: `just test-capability-boot` — the boot domain parks on `Notification.Wait`, the debug-gated Bounce fixture domain signals `0xaa` and parks, and the boot domain resumes with the delivered bits — plus `just test-object-host` (76), `just test-untyped`, `just test-key-table`, `just test-debug-console`, `just fmt-check`, `just clippy`.)
  - [x] Implement the pending-invocation state core: bounded kernel-private records with non-wrapping generation identities (the closed-wait identity), the enforced single-terminal-transition rule, bounded FIFO wait queues with validate-before-reserve admission and `abandon` rollback, and the Notification object state core (coalescing bitmap, one-consumer delivery, teardown cancellation). (2026-09-16: `objects/completion.rs` + `objects/notification.rs`, 14 model tests running in the existing embedded harnesses; `Nucleus` gained the pending pool. The dispatch-level blocked outcome, per-domain saved contexts, and scheduling landed on 2026-09-18 (parent item above); the Signal authority decision (D4), the Notification Retype arm and handler, entry-path blocked handling, the debug-gated Bounce fixture domain, and the end-to-end boot test all followed. Validation: `just build rpi3 qemu`, `just test-untyped` (43 tests), `just test-key-table` (41), `just test-debug-console` (38), `just test-capability-boot`, `just fmt-check`, `just clippy`.)
- [ ] Specify bounded wait/reply resource reservation and cancellation on domain/capability teardown.
  - [x] Bounded reservations exist (pending-pool capacity, per-object wait-queue capacity with before-admission rejection) and object-level teardown cancellation is implemented for Notification and EventCount. (2026-09-16/2026-09-18; domain-teardown-driven cancellation of all pending records remains open.)
...
- [ ] Finalize badge/caller-bit authority and notification waiter consumption versus broadcast; reconcile pending summaries with DCB indexing (D4/D5/D7). (2026-09-16: badge/caller-bit authority selected — the sketch's hybrid: badge bits when the capability is badged, caller-supplied argument otherwise, with `SEND`/`RECV` per-kind rights; waiter consumption selected — one consumer. Retype installs badge zero, so the argument path is the initially reachable one; badge derivation (D4) activates the badge path. DCB pending-summary reconciliation remains open.)
- [x] Define shared-payload memory ordering for signal/advance and observation, including already-satisfied waits, polling, and buffer-slot reuse; distinguish DMA/cache-coherency requirements (D7). (2026-09-18: kernel-mediated release/acquire selected — a Signal/Advance acts as a release, observing the produced value/bitmap through Wait/Poll/Await/Read acts as an acquire, covering blocking wakeups, already-satisfied waits, polling, and future buffer-slot reuse; provided by the kernel-mediated operation (trap/lock serialization on the current single-core kernel; Acquire/Release kernel atomics become a multicore obligation); DMA/device completion explicitly excluded. Recorded in the contract's communication section and D7.)
- [x] Implement Notification Signal/Wait/Poll with checked results, authorization, and race-free wait registration, consumption, wakeup, and cancellation. (2026-09-18: all three operations are active through real SVC dispatch — the already-satisfied wait consumes pending bits, a would-block wait parks and resumes with the delivered bitmap, and wakeup follows the enforced single terminal transition with incarnation-checked waiter delivery. Rights denial is kernel-checked but not exercised in boot: Notification is not on the CopyDerive allowlist, so no attenuated capability can be built there yet. Finite timeouts remain rejected until the time subsystem exists; domain-teardown-driven cancellation of a domain's pending records remains open under the reservation item above. Validation: `just test-capability-boot`, `just test-object-host` (76), `just test-untyped`, `just test-key-table`, `just test-debug-console`, `just fmt-check`, `just clippy`.)
  - [x] Implement the non-blocking core: Notification is Retype-creatable from the bootstrap-carved pool (no Untyped bytes; capability is a checked pool identity), `Signal` and `Poll` are active through real SVC dispatch with `SEND`/`RECV` authority and the badge-hybrid bits, and `Wait`'s already-satisfied path consumes and returns pending bits. (2026-09-16: a wait that would block returned a defined error until the entry-path blocked handling landed on 2026-09-18 — no fake success; finite timeouts are rejected until the time subsystem exists. Rights denial is checked in the kernel but not exercised in boot: Notification is not on the CopyDerive allowlist, so no attenuated capability can be built there yet. Validation: `just test-object-host` (76, including 8 new notification decoder/wrapper tests), `just test-untyped` (46, including notification Retype success/exhaustion-rollback/size tests), `just test-capability-boot` (Retype + Signal/Poll/Wait through the real SVC path), `just test-key-table`, `just test-debug-console`, `just fmt-check`, `just clippy`. En route, the strict wire-argument convention was repaired for `protected_call1..5`: unused argument registers are now transmitted as zero, as the 2026-09-15 selection always required — only `protected_call0` had the fix.)
- [x] Define EventCount overflow behavior and implement Advance/Await/Read with monotonic progress, independent readers, checked arithmetic, and race-free threshold wakeup. (2026-09-18: overflow policy selected — nonzero delta required; an overflowing advance completes with the shared `CounterOverflow` status 31, leaves the counter unchanged, and error-completes every queued `Await` so waiters observe the producer's failure; never wraps, never saturates. All three operations active through real SVC dispatch with `SEND`/`RECV` authority: broadcast wakeups complete every satisfied `Await` in queue order with the new value, already-satisfied awaits return immediately, awaiting does not consume the counter, and the completed pending record now carries the full result shape (status + two words) so error wakeups resume with their status. Validation: `just test-capability-boot` (Retype + `Advance`/`Await`/`Read` through the real SVC path; a blocking `Await` parked and resumed by the Bounce fixture's advance; an overflow-blocking `Await` resumed with status 31), `just test-object-host` (85, including decoder/wrapper/overflow tests), `just test-untyped` (63, including the event-count model tests and the Retype success/size-rejection/exhaustion-rollback tests), `just test-key-table`, `just test-debug-console`, `just fmt-check`, `just clippy`.)
- [ ] Add client wrappers using shared schemas/decoders; test invalid authority, no-pending poll, signal/wait races, multiple waiters, cancellation, counter overflow, reader lag, and shared-payload publication/reuse ordering. (2026-09-16: the Notification wrapper (`NotificationKey` signal/wait/poll with the shared decoder and `WAIT_INFINITE`) is active with host tests. 2026-09-18: the EventCount wrapper (`EventCountKey` advance/await_ge/read with the shared decoder, `WAIT_INFINITE`, and the `EventCountReader` helper) is active with host tests including counter-overflow propagation. The item stays open for the signal/wait race, multiple-waiter, cancellation, reader-lag, and shared-payload publication/reuse tests that need domain-teardown or multi-domain fixtures.)
