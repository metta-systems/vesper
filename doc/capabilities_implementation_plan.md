...
| `just test-untyped` | Passed 19 QEMU cases: the included storage-model suite plus device-source rejection without state changes, rejection preceding capacity validation and destination resolution, and the RAM contrast reaching capacity validation |
