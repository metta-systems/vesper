...
mapping a frame installs a real page/block descriptor in a target Domain's
translation tables; sharing across protection boundaries is done by mapping
the same physical frame into each participant's own Domain context as
distinct PTEs.
...
|---|---|---|---|---|
| `0` | Map | `x2` target Domain key, `x3` virtual address, `x4` requested rights, `x5` attributes (only zero accepted), `x6..x7` zero | requested rights ⊆ frame rights, `READ` required, `MAP` on the target Domain cap | zeros |
...
- **The explicit target-Domain argument is bootstrap-era mechanism**: it lets
  an authorized builder populate a Domain's address space before that Domain
...
- **Alias policy**: the frame's physical extent must not overlap any live
  mapping in the target Domain, whatever capability installed it
  ...
  the target Domain's root, so cross-Domain aliases remain allowed.
- A frame capability records its single mapping (owning Domain identity +
  ...
  the owning Domain's bound ASID (if any).

## Kernel-level implementation details

The capability *is* the object: `FramePayload` is stored inline in the
`KeyEntry` — physical address, mapping record (Domain identity + vaddr),
...
```mermaid
flowchart TD
    A["Frame.Map"] --> B["Resolve frame + target Domain caps<br/>(caller's table)"]
    B --> C{"Frame unmapped, non-device,<br/>rights subset, MAP on Domain?"}
    C -- "no" --> E["defined error"]
    C -- "yes" --> D["Alias-policy walk of<br/>target Domain's tables"]
...
- Unmap verifies the descriptor still points at this frame before clearing
  ...
  address under the owning Domain's ASID — observable on the live context
...
- The same physical frame may be mapped by different derived caps at different
  virtual pages in *different* Domains (same-address sharing preferred for
...
- `Vesper.md` (vault): single-address-space "sharing a buffer is nearly as
  ...
  participant's Domain; numeric pointer possession grants nothing. The vault
...
- `Vesper Capabilities (from wiki).md` (vault): "protection domains …
  ...
  Domain's `MAP` authority; there is no "access from the outside" operation
